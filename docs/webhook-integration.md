...
cc-company webhook set-secret your-webhook-secret
```
...
`.cc-company/agents/developer/agent.json`:
...
cc-company webhook set-approve-condition any

# 모든 requested reviewer가 approve해야 merge ticket 생성
cc-company webhook set-approve-condition all
```

## Configuration

`.cc-company/config.json`:
